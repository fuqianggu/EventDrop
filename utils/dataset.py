...
def drop_by_area(events, resolution, area_ratio=0):
...
def random_shift_events(events, max_shift=20, resolution=(180, 240), p = 0.5):
    H, W = resolution
...
# mode option: 'event_drop', 'drop_by_time', 'drop_by_area', 'random_drop' 
class NCaltech101:
    def __init__(self, root, augmentation=False, resolution=(180, 240), mode='event_drop', ratio=0):
    ...
    def __getitem__(self, idx):
        ...
        if self.augmentation:
            if 'event_drop' in self.mode:
                events = event_drop(events, resolution=self.resolution)
            elif np.random.random() <=0.5:
                if 'drop_by_time' in self.mode:
                    events = drop_by_time(events, T=self.ratio)
                elif 'drop_by_area' in self.mode:
                    events = drop_by_area(events, resolution=self.resolution, area_ratio=self.ratio)
                elif 'random_drop' in self.mode:
                    events = random_drop(events, ratio=self.ratio)

        return events, label


class NCars:
    def __init__(self, root, augmentation=False, resolution=(120, 100), mode='event_drop', ratio=0):
        self.classes = listdir(root)
    ...
    def __getitem__(self, idx):
        ...
        if self.augmentation:
            if 'event_drop' in self.mode:
                events = event_drop(events, resolution=self.resolution)
            elif np.random.random() <= 0.5: # otherwise, do identity operation
                if 'drop_by_time' in self.mode:
                    events = drop_by_time(events, T=self.ratio)
                elif 'drop_by_area' in self.mode:
                    events = drop_by_area(events, resolution=self.resolution, area_ratio=self.ratio)
...
class NMNIST:
    def __init__(self, root, augmentation=False, resolution=(34, 34), mode='event_drop', ratio=0):
    ...
    def __getitem__(self, idx):
        """
        return events and label, loading events from aedat
        :param idx
        :return: x, y, t, p, label
        """
        ...
        f = self.file_path + self.files[idx]
        events = np.load(f).astype(np.float32)
